"""
Phase B — Generator Hierarchy G0→G6 for SuperEnalotto
======================================================
Fits 7 candidate generative models on the historical data and compares them via:
  - Log-likelihood (Bernoulli proxy at per-(draw,number) cell — invariant assumption
    violated equally across all models, so ordering is consistent)
  - AIC / BIC
  - Brier score
  - Walk-forward predictive hits@6
  - Posterior predictive checks on summary statistics
  - PRNG reference comparison (MT19937, PCG64, Philox)

Models
------
G0  Uniform without replacement (physical null)
G1  PRNG references (MT19937, PCG64, Philox) — same distribution as G0,
    used only for posterior-predictive comparison to show that observable
    deviations are not artefacts of any specific PRNG family.
G2  Dirichlet-Multinomial with weak prior (per-number micro-bias)
G3  Lag-1 binary Markov per number (sequential memory)
G4  Structural-feature GLM (decade × parity × fascia bias)
G5  G2 split by regime (pre-Flutter, Flutter, post-4-week)
G6  Weighted ensemble of G2/G3/G4

Output
------
inference/phaseB_model_selection.csv
inference/phaseB_ppc.json
inference/phaseB_summary.json
"""
...
INITIAL_TRAIN = 400
REFIT_EVERY = 50
PPC_SIMS_PER_DRAW = 1  # one simulation per test step (we have many test steps)
...
# Regime indicator (per CLAUDE.md change-points)
...
def summary_stats(draws_list):
    ...
    for d in draws_list:
        s = sorted(d)
        consec_pairs.append(sum(1 for i in range(1, 6) if s[i] == s[i - 1] + 1))
...
# ---------------- MODELS ----------------
# qmat[t, i] = P(number i+1 drawn at draw t | data 0..t-1)
# Normalization: sum_i qmat[t, i] ≈ 6 (expected count per draw)
# For t < INITIAL_TRAIN the default uniform 6/90 is used.
...
def qmat_G3(lag=1, alpha=1.0):
    """Per-number lag-1 binary Markov.
    P(i drawn at t | i drawn at t-1) and P(i drawn at t | NOT drawn at t-1).
    Uses only data 0..t-1 to estimate transitions.
    """
    q = np.full((N, 90), 6.0 / 90.0)
    start = max(lag, INITIAL_TRAIN)
    for t in range(start, N):
        for i in range(90):
            prev_arr = draw_mat[lag - 1:t - 1, i]
            curr_arr = draw_mat[lag:t, i]
            n11 = int(((prev_arr == 1) & (curr_arr == 1)).sum())
            n10 = int(((prev_arr == 1) & (curr_arr == 0)).sum())
            n01 = int(((prev_arr == 0) & (curr_arr == 1)).sum())
            n00 = int(((prev_arr == 0) & (curr_arr == 0)).sum())
            prev_at_t = draw_mat[t - 1, i]
            if prev_at_t == 1:
                p = (n11 + alpha) / (n11 + n10 + 2 * alpha)
            else:
                p = (n01 + alpha) / (n01 + n00 + 2 * alpha)
            q[t, i] = p
        # Normalize so sum(q[t]) = 6 (preserve expected count)
        s = q[t].sum()
        if s > 0:
            q[t] = q[t] / s * 6.0
    return q
...
def qmat_G4(refit_every=REFIT_EVERY):
    """GLM with structural features (decade, parity, fascia). Renormalized to E[count]=6."""
    q = np.full((N, 90), 6.0 / 90.0)
    nums = np.arange(1, 91)
    decade = (nums - 1) // 10
    parity = nums % 2
    fascia = np.where(nums <= 30, 0, np.where(nums <= 60, 1, 2))
    dec_d = np.eye(9)[decade]            # (90, 9)
    par_d = np.column_stack([1 - parity, parity])  # (90, 2)
    fas_d = np.eye(3)[fascia]            # (90, 3)
    ...
    for t in range(INITIAL_TRAIN, N):
        ...
        # Renormalize to E[count] = 6
        p = p / p.sum() * 6.0
        q[t] = np.clip(p, 1e-6, 1 - 1e-6)
    return q

def qmat_G5(alpha=1.0):
    """G2 fitted within the current regime only."""
    q = np.full((N, 90), 6.0 / 90.0)
    for t in range(1, N):
        reg = regimes[t]
...
print("\n[EVAL]")
results = []
results.append(evaluate("G0_uniform",       q_G0, k_params=0))
results.append(evaluate("G2_dirichlet",     q_G2, k_params=90))
results.append(evaluate("G3_markov_lag1",   q_G3, k_params=180))
results.append(evaluate("G4_structural",    q_G4, k_params=14))
results.append(evaluate("G5_regime_split",  q_G5, k_params=270))
results.append(evaluate("G6_ensemble",      q_G6, k_params=286))
...
# Paired permutation test: best vs G0
...
# ---------------- EXPORT ----------------
res_df.to_csv(OUT / "phaseB_model_selection.csv", index=False)
perm_df.to_csv(OUT / "phaseB_permutation_test.csv", index=False)
with open(OUT / "phaseB_ppc.json", "w", encoding="utf-8") as f:
    json.dump({"obs_stats_test": obs_stats_test, "models": ppc_results}, f, indent=2, ensure_ascii=False)
with open(OUT / "phaseB_summary.json", "w", encoding="utf-8") as f:
    json.dump({
        "n_extractions": N,
