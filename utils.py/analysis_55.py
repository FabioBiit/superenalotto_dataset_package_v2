"""
SuperEnalotto 5.5 anni - Analisi statistica rigorosa.
Stesso framework dell'analisi precedente, output: TOP-5 combinazioni.
"""
...
CSV_PATH = r"C:\Users\kyros\OneDrive\Desktop\superenalotto_dataset_package_v2\superenalotto_dataset_package_v2\data\superenalotto_5.5_anni_extraction.csv"
...
dq_results.append(("Valori nulli", "OK" if null_cnt == 0 else "FAIL", f"{null_cnt} valori nulli", "Bassa" if null_cnt == 0 else "Alta"))
...
dq_results.append(("Sestine duplicate (set)", "OK" if dup_sestine == 0 else "WARN", f"{dup_sestine} sestine duplicate", "Bassa" if dup_sestine == 0 else "Media"))

def out_of_range(s, lo=1, hi=90):
    return ((s < lo) | (s > hi)).sum()
oor_main = sum(out_of_range(df[c]) for c in MAIN_COLS)
dq_results.append(("Range numeri 1..90", "OK" if oor_main == 0 else "FAIL", f"{oor_main} fuori range", "Bassa" if oor_main == 0 else "Alta"))

oor_jolly = out_of_range(df["jolly"])
dq_results.append(("Range Jolly 1..90", "OK" if oor_jolly == 0 else "FAIL", f"{oor_jolly} fuori range", "Bassa" if oor_jolly == 0 else "Alta"))

oor_ss = out_of_range(df["superstar"])
dq_results.append(("Range SuperStar 1..90", "OK" if oor_ss == 0 else "FAIL", f"{oor_ss} fuori range", "Bassa" if oor_ss == 0 else "Alta"))

def has_internal_dup(row):
    nums = [row[c] for c in MAIN_COLS]
    return len(nums) != len(set(nums))
internal_dup = df.apply(has_internal_dup, axis=1).sum()
dq_results.append(("Sestine con duplicati interni", "OK" if internal_dup == 0 else "FAIL", f"{internal_dup}", "Bassa" if internal_dup == 0 else "Critica"))

n_extractions = len(df)
date_min = df["date"].min().strftime("%Y-%m-%d")
date_max = df["date"].max().strftime("%Y-%m-%d")
dq_results.append(("N. estrazioni", "INFO", f"{n_extractions} dal {date_min} al {date_max}", "Info"))
...
for yr, grp in df.groupby("year"):
    ...
    if not all(contests[i] < contests[i+1] for i in range(len(contests)-1)):
        contest_anomalies += 1
dq_results.append(("Monotonia contest_number per anno", "OK" if contest_anomalies == 0 else "WARN", f"{contest_anomalies} anomalie", "Bassa" if contest_anomalies == 0 else "Media"))

dup_dates = df["date"].duplicated().sum()
dq_results.append(("Date duplicate", "OK" if dup_dates == 0 else "WARN", f"{dup_dates}", "Bassa" if dup_dates == 0 else "Media"))

dq_results.append(("Colonne winners_6/prize_6/source_url", "WARN", "Assenti nel CSV nonostante schema dichiarato le includa", "Media"))

print("=== DATA QUALITY ===")
for r in dq_results: print(r)

# ============================== FREQUENZE ==================================
all_nums = df[MAIN_COLS].values.flatten()
freq = Counter(all_nums.tolist())
total_draws = sum(freq.values())

freq_df = pd.DataFrame({
    "numero": list(range(1, 91)),
    "frequenza": [freq.get(i, 0) for i in range(1, 91)]
})
freq_df["pct"] = freq_df["frequenza"] / total_draws * 100
freq_df = freq_df.sort_values("frequenza", ascending=False).reset_index(drop=True)

top10 = freq_df.head(10)
bot10 = freq_df.tail(10).iloc[::-1]

n_even = sum(1 for x in all_nums if x % 2 == 0)
n_odd = sum(1 for x in all_nums if x % 2 != 0)

def fascia(x):
    if x <= 30: return "Bassa"
...
print(f"\nPari: {n_even} ({n_even/(n_even+n_odd)*100:.2f}%), Dispari: {n_odd} ({n_odd/(n_even+n_odd)*100:.2f}%)")
print(f"Fasce: {dict(fascia_counts)}")
print(f"Decine: {dict(sorted(decile_counts.items()))}")

# ============================== RITARDI ===================================
df_sorted = df.sort_values(["date", "contest_number"]).reset_index(drop=True)
N = len(df_sorted)

last_seen = {i: None for i in range(1, 91)}
ritardi_correnti = {}
...
for num in range(1, 91):
    ...
    ritardo_medio[num] = float(np.mean(ritardi_storici_per_num[num])) if ritardi_storici_per_num[num] else float("nan")
...
rit_df_sorted = rit_df.sort_values("ritardo_attuale", ascending=False).reset_index(drop=True)

top_ritardatari = rit_df_sorted.head(15).copy()
...
last_156 = df_sorted.tail(156)  # ~1 anno

def freq_subset_pct(sub):
    nums = sub[MAIN_COLS].values.flatten()
    c = Counter(nums.tolist())
    s = sum(c.values())
    return {k: v/s*100 for k, v in c.items()}

freq_total_pct = {i: freq.get(i,0)/total_draws*100 for i in range(1,91)}
freq_36_pct = freq_subset_pct(last_36)
freq_72_pct = freq_subset_pct(last_72)
freq_156_pct = freq_subset_pct(last_156)
...
for yr, grp in df_sorted.groupby("year"):
    nums_yr = grp[MAIN_COLS].values.flatten()
    year_freq[int(yr)] = len(nums_yr)

print(f"\nScaldati 36: {scaldati}")
print(f"Raffreddati 36: {raffreddati}")
print(f"Estrazioni per anno: {year_freq}")
...
n_pair_expected = N * 15 / (90*89/2)
n_triple_expected = N * 20 / (90*89*88/6)

print(f"\nAtteso coppia uniforme: {n_pair_expected:.3f}")
print(f"Atteso tripla uniforme: {n_triple_expected:.3f}")
print(f"Top coppie: {top_pairs[:10]}")
print(f"Top triple: {top_triples[:10]}")

# ============================== TEST CASUALITA ============================
observed = np.array([freq.get(i, 0) for i in range(1, 91)], dtype=float)
expected = np.full(90, total_draws / 90)
...
# Monte Carlo: limito a 2000 simulazioni per N grande (1133)
M = 2000
mc_chi2 = []
...
for _ in range(M):
    ...
    expected_sim = np.full(90, len(sim)/90)
...
print(f"\nChi2 osservato: {chi2_stat:.3f} (critico 5% = {chi2_critical_05}, 1% = {chi2_critical_01})")
print(f"Entropia: {entropy_bits:.4f}/{entropy_max:.4f} = {entropy_bits/entropy_max*100:.2f}%")
print(f"P-value MC: {p_value_empirical:.4f}")
print(f"MC: mean={mc_chi2.mean():.2f}, std={mc_chi2.std():.2f}, p95={np.percentile(mc_chi2,95):.2f}")
...
def has_pattern_issues(combo):
    ...
    if all(x <= 30 for x in s) or all(x >= 61 for x in s): return True
    return False

def strat_frequenza(seed):
    rng = random.Random(seed)
    top_pool = [n for n, _ in sorted(freq_map.items(), key=lambda x: -x[1])[:30]]
    mid_pool = [n for n, _ in sorted(freq_map.items(), key=lambda x: -x[1])[25:55]]
    for _ in range(500):
...
def strat_ritardatari(seed):
    ...
    for _ in range(500):
        ...
        if not has_pattern_issues(c): return c
...
def strat_antipattern(seed):
    ...
    for _, row in df_sorted.tail(10).iterrows():
        for c in MAIN_COLS: recent_nums.add(row[c])
    ...
    for _ in range(500):
        ...
        if len(bassi) < 2 or len(medi) < 2 or len(alti) < 2:
            bassi = list(range(1, 31)); medi = list(range(31, 61)); alti = list(range(61, 91))
...
# Genera 6 combinazioni per strategia -> 30 candidate, poi prendo top-5
combos = []
for i in range(6):
...
def pick_jolly_ss(combo, seed):
    ...
    j = rng.choice(jolly_pool) if jolly_pool else rng.randint(1, 90)
    s = rng.choice(ss_pool) if ss_pool else rng.randint(1, 90)
    return j, s

# Scoring
recent_nums_set = set()
for _, row in df_sorted.tail(10).iterrows():
    for c in MAIN_COLS: recent_nums_set.add(row[c])
...
for i, (strat, combo) in enumerate(combos):
    ...
    rows.append({
        "strategia": strat, "combinazione": tuple(sorted(combo)),
        "jolly": int(j), "superstar": int(ss), "score": sc,
...
# Deduplica eventuali combinazioni identiche tenendo solo la migliore
out_df = out_df.drop_duplicates(subset=["combinazione"]).reset_index(drop=True)
out_df["rank"] = out_df.index + 1
out_df["combinazione_str"] = out_df["combinazione"].apply(lambda c: "-".join(f"{x:02d}" for x in sorted(c)))

# Export CSV completo (tutte) ma report mostra solo top 5
out_path = r"C:\Users\kyros\OneDrive\Desktop\superenalotto_dataset_package_v2\superenalotto_dataset_package_v2\combinazioni_ranking_55.csv"
out_df[["rank","strategia","combinazione_str","jolly","superstar","score","n_even","n_dec","rit_avg","freq_avg"]].to_csv(out_path, index=False)

print(f"\nRanking esportato: {out_path}")
print("\n=== TOP 5 ===")
print(out_df[["rank","strategia","combinazione_str","jolly","superstar","score","n_even","n_dec","rit_avg","freq_avg"]].head(5).to_string(index=False))
...
summary = {
    ...
    "chi2_stat": chi2_stat,
    "chi2_crit_05": chi2_critical_05, "chi2_crit_01": chi2_critical_01,
    "entropy_bits": entropy_bits, "entropy_max": entropy_max,
    "entropy_eff_pct": entropy_bits/entropy_max*100,
    "mc_p_value": p_value_empirical,
    "mc_mean": float(mc_chi2.mean()), "mc_std": float(mc_chi2.std()), "mc_p95": float(np.percentile(mc_chi2, 95)),
    "n_pair_expected": float(n_pair_expected),
    "n_triple_expected": float(n_triple_expected),
    "top10_freq": top10.to_dict(orient="records"),
    "bottom10_freq": bot10.to_dict(orient="records"),
    ...
    "year_freq": year_freq,
    "jolly_top5": sorted(jolly_freq.items(), key=lambda x: -x[1])[:5],
    "ss_top5": sorted(ss_freq.items(), key=lambda x: -x[1])[:5],
}
with open(r"C:\Users\kyros\OneDrive\Desktop\superenalotto_dataset_package_v2\superenalotto_dataset_package_v2\analysis_summary_55.json", "w") as f:
    json.dump(summary, f, indent=2, default=str)
print("\nSummary 5.5 anni salvato.")
